...
| Event Horizon Crossing Simulation | `0_RB` (the verified fixed anchor `e0`/`e8`) plus `L_(I|O)` (TheInterface's crossing operator) are sufficient to construct a horizon-crossing simulation whose geometry is Israel-junction-matched between a Schwarzschild exterior and a gravastar's de Sitter interior — a real 3D scene, not an illustration. STUB, requirements-only; GPU build deferred. |
