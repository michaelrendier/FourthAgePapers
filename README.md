...
| No Singularity | There is no singularity. The black-hole interior is a finite, sub-Planckian de Sitter core — the Abrikosov vortex core made gravitational (condensate → 0, winding 1, everything else finite). **Holcus:** the maximum curvature is the de Sitter Kretschmann scalar at L_dS = r_s, K_core(M) = (3/2) c⁸/(G⁴M⁴) — M⁻⁴, sub-Planckian for every M > (3/2)^¼ m_Pl, with a ringdown echo at delay ~ r_s/c as its shadow. Falsifier: divergent core curvature, or one pinned to K_Planck independent of M. P1 STANDS; P4 (cosmic budget ≈ Ω_Λ) FALSIFIED as a magnitude — dark-flow signature only. |
